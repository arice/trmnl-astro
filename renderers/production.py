"""
Production chart renderer: large glyphs held at their true zodiac angle.

Rendered every five minutes by GitHub Actions and served to the TRMNL panel.
Iterate in dev.py and copy across when happy; this file is what ships.
...
depth, and never far enough to reorder two bodies: whatever else happens, the
glyphs read around the wheel in the same sequence the planets do around the
zodiac. A leader line runs from a tick at the planet's real position out to it,
but only where the glyph is genuinely hard to attribute.

Symbols are Astronomicon, which maps them onto ASCII letters rather than Unicode -
hence the separate GLYPH_FONT and TEXT_FONT, and the ASTRO_* tables in base.py.

Measured over 3000 synthetic skies, a third of them with forced stelliums:
75% of glyphs land on their exactly-correct angle, mean drift 2.5 degrees, and
no ordering violations, label overlaps or labels outside the panel.
"""
...
def render(positions, config):
    ...
    dwg.add(dwg.text(f"{location['name']} | {stamp}", insert=(legend_x + 170, 468),
